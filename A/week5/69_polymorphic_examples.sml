...
(* We end up with fewer constraints so some arguments can be of any type
  First several steps are just like with sum from previous segment:
   ...
   Since no more constraints we replace consistently all the the remaining T's
   with type variables ('a, 'b etc.)
   so 'a list -> int
*)
fun length xs =
    case xs of
...
(*
   ...
   from body of compose being aanonymous function, T3 = T4->T5 for some T4 and
   T5
