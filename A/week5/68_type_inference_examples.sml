...
(* We look first at "fun f x ="
   for some types T1 and T2, f has to have type T1 -> T2
   f : T1 -> T2 [must be a function; all functions take one argument]
   x : T1 [must have type of f's argument, the patterm for the argument is here
a variable x]

We look at function body for additional constraints:
"let val (y,z) = x in"
   y : T3 
   z : T4 
...
Look at function body where we pattern match a T1 (xs) "x::xs" only matches
list so 
