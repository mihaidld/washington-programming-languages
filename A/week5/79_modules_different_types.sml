...
(*Even though modules can be equivalent under same signature, we can not
 mix and match because each module defines different rational type:
 Rational1.rational is used by functions in Rational1 module, but
 Rational2.rational is used by functions in Rational2 module

It is good since different modules have different internal invariants
e.g. Rational1.toString(Rational2.make_frac(9,6)) would return "9/6" if it would
type check and not expected "3/2" in reduced format since both functions don't
reduce*)
