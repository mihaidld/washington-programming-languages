...
(* exp -> exp*)
(* preprocesses expression to remove all negative constants by making a
different expression *)
fun noNegConstants e =
    case e of
	(* if underlying i negative, then replace expression Int i with Negate
	   of an Int holding a positive i
	   E.g. noNegConstants(Int -3) -> Negate (Int 3) *)
	Int i       => if i < 0 then Negate (Int(~i)) else e
      (* recursively remove all negative constants from subexpression e1*) 
