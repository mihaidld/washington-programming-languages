...
in
	...
	then (print "preprocess flips an improper LineSegment x successfully\n")
	else (print "preprocess does not flip an improper LineSegment x successfully\n")
end;
...
in
	if real_equal(a,e) andalso real_equal(b,f) andalso real_equal(c,g) andalso real_equal(d,h)
	then (print "preprocess flips an improper LineSegment y successfully\n")
	else (print "preprocess does not flip an improper LineSegment y successfully\n")
end;

let 
	val Intersect(_,LineSegment(a,b,c,d)) = preprocess_prog (Intersect( NoPoints, (LineSegment(3.2,4.1,3.2,~4.1))))
	val LineSegment(e,f,g,h) = LineSegment(3.2,~4.1,3.2,4.1)
in
	if real_equal(a,e) andalso real_equal(b,f) andalso real_equal(c,g) andalso real_equal(d,h)
	then (print "preprocess flips recursively an improper LineSegment y successfully\n")
	else (print "preprocess does not flip an improper LineSegment y successfully\n")
end;
...
let 
    val a = (eval_prog (preprocess_prog (Shift(3.0, 4.0, NoPoints)), []))	
in
    case a of
	NoPoints => (print "shift NoPoints worked\n")
      | _  => (print "shift NoPoints is not working properly\n")
end;

let 
	val Line(a,b) = (eval_prog (preprocess_prog (Shift(3.0, 6.0, Line(4.0,5.0))), []))
	val Line(c,d) = Line(4.0,6.0 + 6.0 - 4.0 * 3.0) 
in
	if real_equal(a,c) andalso real_equal(b,d)
	then (print "shift Line worked\n")
	else (print "shift Line is not working properly\n")
end;

let 
	val VerticalLine(a) = (eval_prog (preprocess_prog (Shift(3.0, 6.0, VerticalLine(4.0))), []))
	val VerticalLine(c) = VerticalLine(7.0) 
in
	if real_equal(a,c)
	then (print "shift VerticalLine worked\n")
	else (print "shift VerticalLine is not working properly\n")
end;
...
in
	if real_equal(a,e) andalso real_equal(b,f) andalso real_equal(c,g) andalso real_equal(d,h)
	then (print "shift LineSegment worked\n")
	else (print "shift LineSegment is not working properly\n")
end;
