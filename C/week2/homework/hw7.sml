...
(* Add shift expressions as defined above to the SML implementation by adding
the constructor Shift of real * real * geom_exp to the definition of geom_exp
and adding appropriate branches to eval_prog and preprocess_prog.
(The first real is deltaX and the second is deltaY .) Do not change other
functions. In particular, there is no need to change intersect because this
function is used only for values in our geometry language and shift expressions
...
(*
A Shift expression is not a value. It has a deltaX (a floating-point number),
a deltaY (a floating-point number), and a subexpression. The semantics is to
evaluate the subexpression and then shift the result by deltaX (in the
x-direction; positive is "to the right") and deltaY (in the y-direction;
positive is "up").
More specifically, shifting for each form of value is as follows:
- NoPoints remains NoPoints.
- A Point representing (x; y) becomes a Point representing (x + deltaX ; y +
deltaY ).
- A Line with slope m and intercept b becomes a Line with slope m and an
intercept of b + deltaY - m * deltaX .
- A VerticalLine becomes a VerticalLine shifted by deltaX ; the deltaY is
irrelevant.
...
fun eval_prog (e,env) =
	...
	(* eval_prog returns value so we need only 5 cases*)
...
(* Implement an SML function preprocess_prog of type geom_exp -> geom_exp to
implement expression preprocessing as defined above. The idea is that evaluating
program e would be done with (eval_prog (preprocess_prog e, []) where the [] is
the empty list for the empty environment.*)
...
fun preprocess_prog e =
	...
	then LineSegment(x2,y2,x1,y1) (* invert points for first lower x-value*)
	else if real_close(x1,x2) andalso y1 > (y2 + epsilon)
	then LineSegment(x2,y2,x1,y1) (* invert points for first lower y-value*)
	else e (* keep it*)
      | NoPoints => e
      | Point _ => e
      | Line _ => e
      ...
      (* if we reach Var case it will raise exception since
	 eval_prog (preprocess_prog e, []) starts with [] empty environment.
	 then in let expressions we extend environment with values in correct
	 order *)
      | Var s => Var s
