...
class Piece 
  ...
  def self.rotations (point_array)
    rotate1 = point_array.map {|x,y| [-y,x]}  #rotate 90deg counter-clock
    rotate2 = point_array.map {|x,y| [-x,-y]} #rotate 180deg
    rotate3 = point_array.map {|x,y| [y,-x]}  #rotate 270deg counter-clock
  ...
  # class constant array holding all the pieces and their rotations
...
class Board
  ...
  # using while loop (if no key events to change x, y or rotation)
  # Then replaces it with a new piece
  # Change the score to reflect the distance dropped (delta score = total change
  # in y) because game is played faster (awards speed)
  def drop_all_the_way
    if @game.is_running?
      ...
      draw #updates current_position
      store_current
      if !game_over?
        next_piece
      end
      @game.update_score
      draw # why call draw 2 times?
    end
  end
  ...
  def store_current
    locations = @current_block.current_rotation # rotation of piece
    displacement = @current_block.position #[x,y] of piece's base position
    #iterate 4 times from index 0 to 3 (because 4 blocks in each initial piece)
    (0..3).each{|index| 
      current = locations[index];
      #current[1]+displacement[1] gets block's y coordinate for row number
      #current[0]+displacement[0] gets block's x coordinate for column number
      #change for each block values in grid from nil to what was stored in
      #current_pos
      @grid[current[1]+displacement[1]][current[0]+displacement[0]] = 
      @current_pos[index]
    }
    #try to remove filled rows
    remove_filled
    @delay = [@delay - 2, 80].max # why change delay?
  end

  # Takes a point [x,y] and checks to see if it is in the bounds of the board
  # and currently empty.
  def empty_at (point)
    if !(point[0] >= 0 and point[0] < num_columns) #0<=x<num_colums
      return false #out of bounds horizontally
    elsif point[1] < 1 #x ok, y=0 always empty since game_over checks from y=1
      return true #return empty without putting nil in grid at [x,y]
    elsif point[1] >= num_rows #y>=num_rows
      return false #out of bounds vertically (too low)
    end
    #we reach this when x,y in the bounds, check if value there is nill
  ...
  def remove_filled
    #range from 2 to num_rows-1 (for rows from 2 to last) -> iterate on all rows
    #except first 2 on top
    (2..(@grid.size-1)).each{|num| row = @grid.slice(num);
      # row stores row at that row_number from grid
      # see if this row is full (has no nil)
      if @grid[num].all? # same as row.all?
        # remove from canvas blocks in full row and replace it with nill
        # create range for each column in filled row: 0<=index<num_colums
        (0..(num_columns-1)).each{|index|
          @grid[num][index].remove;#grid[row_number][column_number] remove
          @grid[num][index] = nil 
        }
        # move down all rows above and move their blocks on the canvas
        # e.g.current row filled is 10 -> create range from 27-10+1 to 27 ->
        # (18..27) -> 18<=num2<=27
        # could have used range (num-1..0) for all rows above num
        ((@grid.size - num + 1)..(@grid.size)).each{|num2|
          # iterate on all columns in rows from 9 (27-18) to 0 (27-27) =
          # all rows above currently filed one 10
          # rect is block at each column in rows above num
          # we move rect only if not nill (rect && ...)
          # we move rect down by 15 pixels (block_size)
          @grid[@grid.size - num2].each{|rect| rect && rect.move(0, block_size)};
          # store in row below the one that was just lowered (27-num2+1) a new
          # Array copy with values that were in row lowered (grid.size-num2)
...
class Tetris
  ...
  # creates the lightblue window and starts the game
  def initialize
    @root = TetrisRoot.new
  ...
  # creates a grey canvas and the board that interacts with it
  def set_board
    @canvas = TetrisCanvas.new
    @board = Board.new(self)
    #canvas is grey rectangle where pieces are displayed
    #height of canvas = 15 * 27 + 3 = 408 (window height 615)
    #width of canvas = 15 * 10 + 6 = 156 (window width 205)
    #x for canvas is 24, y canvas 80 to place canvas in middle of window
    #horizontally (x=(205-156)/2=24) and a little above middle vertically
    #(y=(615-408)/2 = 103) to leave place for keyboard instructions
    @canvas.place(@board.block_size * @board.num_rows + 3,
                  @board.block_size * @board.num_columns + 6, 24, 80)
    @board.draw
  end

  # keyboard bindings with callbacks
  # put method in proc to avoid callling it right away
  # e.g. if press key n call method new_game (start new game) 
  def key_bindings  
  ...
  def buttons
    # create button for pause with label "pause", background color "lightcoral"
    # and command when clicking it the proc which calls game's pause method
    pause = TetrisButton.new('pause', 'lightcoral'){self.pause}
    #place button at absolute position ?
    ...
    #create TkLabel with block parameter options (prefixed with &) for
    #text and background
    label = TetrisLabel.new(@root) do
      text 'Current Score: '    #fixed text for label "Current Score"
    ...
    @score.text(@board.score) #dynamic text (based on Board's score)
  ...
  def new_game
    @canvas.unplace
    @canvas.delete
    #create new canvas and board
    set_board
    #reset displayed score to 0
  ...
  def pause
    #toggles value of running instance variable
    if @running
      @running = false
      @timer.stop
    else
  ...
  def draw_piece (piece, old=nil)
    ...
    #for each one of the block of the piece it creates a TkcRectangle with
    #current canvas, a is x coordinate of block * 15 pixels + 3,
    #b is y coordinate of block * 15 pixels + 3, c is a + 15, d is b + 15,
    #piece's color
    blocks.map{|block| 
