...
(* this way is more painful here, but makes much more sense for 
   ...
   Racket can return a string or int or ...
, while ML must always return an int
*)
exception Error of string

fun eval_exp_new e =
    (* helper function to check if e holds an int, if so produce it,
    ...
    in
	case e of
            Const _ => (* Base case: if already a constant, return expression*)
	    e
	  (* make recursive calls on e1,e2,
	     get the underlying data (int) with get_int,
	     perform some operation on it e.g. ~/+/*,
	     make again exp with Const to produce result*)
	  | Negate e2  => 
